...
import org.springframework.beans.factory.annotation.Value; import org.springframework.http.MediaType; import org.springframework.stereotype.Component; import org.springframework.web.client.*; import java.util.*;
@Component
public class OrchestratorClient {
 private final RestClient client;
 public OrchestratorClient(RestClient.Builder builder,@Value("${orchestrator.base-url}") String baseUrl){this.client=builder.baseUrl(baseUrl).build();}
 public StartResponse start(UUID projectId,String threadId,String name,String idea){try{return client.post().uri("/internal/workflows").contentType(MediaType.APPLICATION_JSON).body(new StartRequest(projectId.toString(),threadId,name,idea)).retrieve().body(StartResponse.class);}catch(RestClientException e){throw new OrchestratorException("Orchestrator start failed: "+e.getMessage(),e);}}
 public ResumeResponse resume(String threadId,String selectedOption,String comment){try{return client.post().uri("/internal/workflows/{threadId}/resume",threadId).contentType(MediaType.APPLICATION_JSON).body(new ResumeRequest(selectedOption,comment)).retrieve().body(ResumeResponse.class);}catch(RestClientException e){throw new OrchestratorException("Orchestrator resume failed: "+e.getMessage(),e);}}
