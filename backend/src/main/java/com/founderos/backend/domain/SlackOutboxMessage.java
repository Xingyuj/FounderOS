...
public class SlackOutboxMessage {
    ...
    @Enumerated(EnumType.STRING) @Column(nullable = false, length = 20) private SlackDeliveryOperation operation;
    @Column(name = "target_message_ts", length = 40) private String targetMessageTs;
    ...
    public SlackOutboxMessage(String teamId, String channelId, String threadTs, AgentRole role, String text, String blocksJson) {
        this.id = UUID.randomUUID(); this.slackTeamId = teamId; this.slackChannelId = channelId; this.slackThreadTs = threadTs; this.agentRole = role; this.text = text; this.blocksJson = blocksJson; this.operation = SlackDeliveryOperation.POST; this.status = QueueStatus.PENDING; this.availableAt = Instant.now(); this.createdAt = this.updatedAt = Instant.now();
    }
    public static SlackOutboxMessage update(String teamId, String channelId, String messageTs, AgentRole role, String text, String blocksJson) { SlackOutboxMessage message = new SlackOutboxMessage(teamId, channelId, null, role, text, blocksJson); message.operation = SlackDeliveryOperation.UPDATE; message.targetMessageTs = messageTs; return message; }
    ...
    public UUID getId() { return id; } public String getSlackTeamId() { return slackTeamId; } public String getSlackChannelId() { return slackChannelId; } public String getSlackThreadTs() { return slackThreadTs; } public AgentRole getAgentRole() { return agentRole; } public String getText() { return text; } public String getBlocksJson() { return blocksJson; } public SlackDeliveryOperation getOperation() { return operation; } public String getTargetMessageTs() { return targetMessageTs; } public QueueStatus getStatus() { return status; } public int getAttemptCount() { return attemptCount; } public Instant getAvailableAt() { return availableAt; } public String getSlackMessageTs() { return slackMessageTs; }
}
